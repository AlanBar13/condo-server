...
import { NotificationStatus, NotificationToken } from './entities/notification-token.entity';
import { User } from 'src/users/entities/user.entity';
import { NotificationDTO, AcceptNotificationDTO, TopicNotificationDTO } from './dto/notification.dto';
...
export class NotificationService {
    ...
    async acceptPushNotification(user: TokenInfo, notificationDto: AcceptNotificationDTO) {
        await this.notificationTokenRepository.update({ userId : user.id }, { status: NotificationStatus.INACTIVE });
    ...
    async disablePushNotification(user: TokenInfo, updateNotificationDto: UpdateNotificationDTO) {
        try {
            await this.notificationTokenRepository.update({ userId : user.id, deviceType: updateNotificationDto.deviceType }, { status: NotificationStatus.INACTIVE });
        } catch (error) {
            Logger.warn(error);
        }
    }
    ...
    async sendSingleNotification(notificationDto: NotificationDTO) {
        try {
            if (notificationDto.userIds.length > 1 || notificationDto.userIds.length === 0){
                throw new BadRequestException("UserIds should be exactly 1")
            }

            const { notificationToken } = await this.notificationTokenRepository.findOne({ where: { id: notificationDto.userIds[0] }});
    ...
    async sendMultipleNotification(notificationDto: NotificationDTO) {
        try {
            ...
            const results = await this.notificationTokenRepository.find({ where: { id: In(notificationDto.userIds), status: NotificationStatus.ACTIVE }});
            ...
            return { success: true, message: `Reaching ${tokens.length} users. Successfully sent ${response.successCount} messages. ${response.failureCount} failed.`};
        } catch (error) {
            Logger.warn(error)
            throw new InternalServerErrorException();
        }
