...
export class VisitantService {
  constructor(@InjectRepository(Visitant) private visitantRepository: Repository<Visitant>) {}
  ...
  findOne(id: string): Promise<Visitant> {
    return this.visitantRepository.findOne({ where: { id },  relations: { house: true }});
  }

  async update(id: string, updateVisitantDto: UpdateVisitantDto): Promise<Visitant> {
